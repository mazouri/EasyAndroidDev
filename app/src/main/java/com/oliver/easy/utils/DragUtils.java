...
public class DragUtils {
    ...
    private static void swapViews(ViewGroup viewGroup, final View view, int index,
        ...
        UIUtils.swapViewGroupChildren(viewGroup, view, dragState.view);
        dragState.index = index;
        UIUtils.postOnPreDraw(view, new Runnable() {
            @Override
            public void run() {
                ObjectAnimator
    ...
    private static void removeView(final ViewGroup viewGroup, DragState dragState) {
        ...
        if (viewGroup.getChildCount() > 0) {
            ...
            UIUtils.setViewLayoutParamsHeight(viewGroup, oldViewGroupHeight);
            // Wait until the OnPreDraw of the last child is called for syncing the two animations on
            // View and ViewGroup.
            UIUtils.postOnPreDraw(viewGroup.getChildAt(viewGroup.getChildCount() - 1), new Runnable() {
                @Override
                public void run() {
    ...
    private static void animateViewGroupHeight(final ViewGroup viewGroup,
        ...
        viewGroupAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                UIUtils.setViewLayoutParamsHeight(viewGroup, animatedValue);
            }
        });
        viewGroupAnimator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                UIUtils.setViewLayoutParamsHeight(viewGroup, oldLayoutParamsHeight);
            }
        });
